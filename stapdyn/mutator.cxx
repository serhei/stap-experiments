...
{
  // TODOXXX This is missing a number of cleanup checks.
  // (A lot of sophistication will have to be added to work with the
  // server idea.)

  // Check if we've received a signal that causes all modules to quit.

  // We'll always break right away for SIGQUIT. XXX We'll also break
  // for any other signal if we didn't create the process.  Otherwise,
  // we should give the created process a chance to finish.
...
mutator::run_to_completion ()
{
  ...
  // XXX May also want this alternate waiting mechanism if there
  // are no targets (trickier to figure out when mutatees come and go).
  //
  // else // !target_mutatee
  //   {
  ...
  if (mut)
    {
      ...
      for (size_t i = 0; i < script_modules.size(); ++i)
        script_modules[i]->post_fork_callback(mut.get(), m.get(), child);
    }
}
          ...
          if (!module_shmem.empty())
            {
              vector<BPatch_snippet *> args;
              args.push_back(new BPatch_constExpr(module_shmem.c_str()));
              t->call_function("stp_dyninst_shm_connect", args);
            }

          // Trigger any process.end probes for the pre-exec process.
          t->exit_callback(thread, true);

          // Trigger any process.begin probes.
          t->begin_callback(thread);
        }
#endif
    }
}
                             ...
                             BPatch_exitType type __attribute__((unused)))
{
  ...
  if (t)
    {
      // FIXME: We'd like to call the target's exit_callback()
      // function, but we've got a problem. The target can't stop the
      // process to call the exit probe within the target (it finishes
      // exiting before we can). So, we'll call the probe(s) locally
      // here. This works, but the context is wrong (the script module, not
      // the target).
      const vector<dynprobe_location>& proc_end_probes =
	t->find_attached_probes(STAPDYN_PROBE_FLAG_PROC_END);
	...
	try
      ...
      staplog(2) << "firing " << proc_end_probes.size()
		 << " process.end probes in the script_module for pid "
		 << pid << endl;
      for (size_t p = 0; p < proc_end_probes.size(); ++p)
        {
	  const dynprobe_location& probe = proc_end_probes[p];
	  staplog(3) << "calling utrace function in the script_module for pid "
